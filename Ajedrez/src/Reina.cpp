			...
			if (posX == j && posY == i) {
				if (j % 2 == 1) {//Bucle para las coordenadas y
					if (i % 2 == 1) {//Bucle para las coordenadas x
						if (color == 'w')
							glBindTexture(GL_TEXTURE_2D, ETSIDI::getTexture("imagenes/2D/reinablanca_fondonegro.png").id);
						if (color == 'b')
							glBindTexture(GL_TEXTURE_2D, ETSIDI::getTexture("imagenes/2D/reinanegra_fondonegro.png").id);
					}
					else {
						if (color == 'w')
							glBindTexture(GL_TEXTURE_2D, ETSIDI::getTexture("imagenes/2D/reinablanca_fondoblanco.png").id);
						if (color == 'b')
							glBindTexture(GL_TEXTURE_2D, ETSIDI::getTexture("imagenes/2D/reinanegra_fondoblanco.png").id);
					}
				}
				else {
					if (i % 2 == 0) {
