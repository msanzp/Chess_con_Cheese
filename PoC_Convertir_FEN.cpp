...
string convierte_a_FEN(string arg[8][8])
{
    ...
    for (n = 0; n < 8; ++n)
    {
        ...
        for (m = 0; m < 8; ++m)
        {
            if (arg[n][m] == "0" && (m != 7)) // Espacio libre en el tablero, se a�ade 1 al contador
            {
                contador += 1;
            }
            else if (arg[n][m] == "0" && (m == 7)) // espacio libre pero es el espacio final, se a�ade 1 al contador y se implementa
            {
                contador += 1;
                respuesta += to_string(contador);
            }
            else if (arg[n][m] != "0" && contador == 0) // Pieza en el tablero y contador 0, se a�ade al string
            {
                if (primera)
                {
            ...
            else // Pieza en el tablero y contador no es 0, se a�ade contador + pieza al string
            {
                respuesta += to_string(contador);
                respuesta += arg[n][m];
                contador = 0;
